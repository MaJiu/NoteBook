...
2. 析构函数  什么也不做
3. 拷贝构造函数 按成员复制 浅拷贝 没有定义拷贝构造函数时提供
4. 重载赋值运算符函数(=) 对应属性赋值 浅拷贝 没有重载复制运算符函数
5. 移动构造函数(C++11)
6. 重载移动赋值运算符函数(C++11)  
...
// 1.构造函数
    ...
    Test &operator=(Test &&) noexcept; // 移动赋值运算符
// 2.重载赋值运算符函数
...
基类声明了一个虚函数，在所有派生类中，即使不再重声明，该函数也是虚函数

虚函数机制在构造函数和析构函数中不起作用，析构函数最好声明为虚函数
...
每当创建一个含有虚函数的类，或从含有虚函数的类派生一个类时，编译器会为这个类创建一个唯一的VTABLE, 在这个 VTABLE 防止该类的所有虚函数的地址。如果在派生称中没有重新定义基类中的虚函数，编译器就使用基类虚函数的地址，然后编译器在这个类中放置VRTR指针成员(指向VTABLE)
...
std:move 函数可以获得绑定到左值的右值引用，调用move表明我们希望把一个左值当作右值来处理，在调用 move 后，**我们不能对移后源对象的值做任何假设**
...
局部类不能使用函数作用域的变量，只能访问外层作用域定义的类型名、变量名、静态变量，以及枚举成员
